...
public class BoardController {
	...
	// "/list" 주소를 가지는 list메서드를 만들어주세요
	// void 리턴을 하며, .addAttribute()를 이용해
	//list 라는 이름으로 전체 글 목록을 뷰에 전달합니다.
	@RequestMapping("/list")
	public void list(Model model,SearchCriteria cri) {
		
		
		model.addAttribute("list", service.getPage(cri));
	...
	//get 기능을 구현해보겠습니다.
	//이 메서드는 파라미토로 글 번호 bno를 입력받고
	//modell.addAttribute() 기능을 이용해 특정 글 번호에
	...
	@PostMapping("/modify")
	public String modify(Long bno, Model model, SearchCriteria cri) {
		model.addAttribute("board",service.get(bno));
		model.addAttribute("cri", cri);
	...
	@PostMapping("/remove")
	public String remove(Long bno,RedirectAttributes rttr,SearchCriteria cri) {
		service.remove(bno);
